...
| `/restore [normal|lite|full] [delta|since] [thinking]` | **복원(전문)** | 원본을 정제해 **주고받은 전체 대화**를 컨텍스트로 되살림 (`since` = 마지막 handoff 이후만) |
...
또한 **압축 직후 재진입**엔 `/restore since` 가 가장 싸다 — `handoff.md` 가 신선하면 그 이후만(마지막 handoff 이후) 정제해 전체를 다시 읽는 재귀 비용을 피한다. 번호 `[N]` 은 전체본과 정합되고 드릴다운 색인도 그대로 붙는다.
