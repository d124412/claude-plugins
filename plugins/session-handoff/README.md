...
| 명령 | 방향 | 설명 |
|------|------|------|
| `/handoff [완료\|중단\|진행중]` | 저장(요약) | 현재 세션의 handoff 파일(.md)과 INDEX를 지금 최신화 |
| `/snapshot` | 저장(원본) | 압축 없이 지금 대화 원본(.jsonl)을 `.archive/`로 즉시 복사 |
| `/restore [full] [thinking]` | **복원(전문)** | 원본을 정제해 **주고받은 전체 대화**를 컨텍스트로 되살림 |
...
모드는 **둘뿐**이다 — **기본은 덤프 빼고, `full`은 전부.** 임의의 절단 길이 같은 매직넘버를 두지 않는다.

| 모드 | 도구 결과 | 도구 호출 인자 | 크기 | 줄수 | 추정 토큰 |
|---|---|---|---|---|---|
| **`/restore`** (기본) | **덤프 제외**(한 줄 미리보기) | **대상만** | 0.31 MB | 5,200 | **~95k** |
| `/restore full` | **전문 보존**(무손실) | **전문 보존** | 0.95 MB | 12,953 | ~290k |

- **어떤 모드든 사용자·Claude 발언은 100% 전문 보존.** 빠지는 건 덤프뿐이고, 그건 파일 내용·명령 출력이라 디스크에서 다시 읽으면 된다.
- **"덤프"는 결과에만 있는 게 아니다.** `Edit` 의 `old/new_string`, `Write` 의 `content`, `Bash` 의 heredoc 본문, `Agent` 의 `prompt` 도 같은 성격이다. 기본 모드는 이것들도 빼고 **대상**만 남긴다:
...
- 정제본은 `.handoff/.archive/<토큰>-full.md` (덮어쓰기). **원본 `.jsonl` 은 직접 읽지 않는다**
...
> - 원본은 **더 완전한 쪽을 자동 선택**
> - 세션 보관 기본 30일(`cleanupPeriodDays`) 이후엔 `.handoff/.archive/` 사본만 남는다
...
└── .archive/
    ...
    ├── <토큰>-fallback.md                    # 무LLM 폴백요약 — 가볍게 훑는 뼈대(덮어쓰기)
    ├── <토큰>-full.md                        # /restore 정제본 — 읽을 수 있는 전체 대화(덮어쓰기)
