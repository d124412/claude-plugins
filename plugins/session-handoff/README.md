...
| `/restore [thinking]` | **복원(전문)** | 원본을 정제해 **주고받은 전체 대화**를 컨텍스트로 되살림 |
| `/update-plugin` | 관리 | 이 플러그인을 마켓 최신 버전으로 업데이트 (`/plugin update` 를 못 쓰는 환경용) |

### `/restore` — 요약이 아니라 "전문"을 되살린다
원본 `.jsonl` 은 `uuid`/`usage`/`requestId`/`cache_*` 같은 **메타데이터가 대부분**이다. 그걸 걷어내면 실측 기준 **5.78 MB → 0.66 MB (▼89%)** 로 줄어, **전체 대화를 통째로 다시 읽어도 컨텍스트에 들어간다.**

- 사용자·Claude 발언은 **전문 보존**, 도구 호출/결과도 전문(접기), 이미지는 자리표시
- 사고과정 블록은 기본 제외 → `/restore thinking` 으로 포함
- 정제본은 `.handoff/.archive/<토큰>-full.md` (덮어쓰기). **원본 `.jsonl` 은 직접 읽지 않는다**(수 MB라 컨텍스트가 터짐)
