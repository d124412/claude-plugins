...
그래서 기본값은 **발언 전문 + 도구 결과 500자 절단**이다:

| 모드 | 크기 | 줄수 | 추정 토큰 | 적합 |
|---|---|---|---|---|
| **`/restore`** (기본) | 0.42 MB | 7,110 | **~139k** | **200k 모델에서도 안전** |
| `/restore full` | 1.12 MB | 18,714 | ~368k | **1M 컨텍스트 전용** |

- **어떤 모드든 사용자·Claude 발언은 100% 전문 보존.** 잘리는 건 도구 결과 덤프뿐이고, 그건 디스크에서 다시 읽으면 된다.
- 500 아래로 줄일 실익 없음(0자로 생략해도 0.34 MB가 하한). 특정 값은 `--tool-result <숫자>` 로 지정 가능.
