...
|---|---|---|---|---|
| **`/restore`** (기본) | **덤프 제외** | 0.37 MB | 4,570 | **~118k** (1M의 12%) |
| `/restore full` | **전문 보존**(무손실) | 1.16 MB | 19,416 | ~362k (1M의 36%) |

- **어떤 모드든 사용자·Claude 발언은 100% 전문 보존.** 빠지는 건 도구 결과 덤프뿐이고, 그건 파일 내용·명령 출력이라 디스크에서 다시 읽으면 된다.
- **덤프를 빼도 "무엇을 했는지"는 남는다** — 도구 이름·인자·결과 한 줄 미리보기가 보존된다.
