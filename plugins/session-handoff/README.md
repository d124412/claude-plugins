...
| `/restore [normal|lite|full] [delta|since] [thinking]` | **복원(전문)** | 원본을 정제해 **주고받은 전체 대화**를 컨텍스트로 되살림. 인자 없는 `/restore` 는 **auto**(handoff 있으면 since, 없으면 normal) |
...
| 모드 | 도구 결과 | 도구 호출 인자 | 발언 | 크기 | 추정 토큰 |
|---|---|---|---|---|---|
| `/restore lite` | — (활동 지도) | — | 전문 | 0.15 MB | **~53k** |
| **`/restore`** (auto) | handoff 신선하면 **since**, 아니면 normal | — | — | 상황에 따라 | **가장 쌈** |
| `/restore normal` | **덤프 제외**(한 줄 미리보기) | **대상만** | 전문 | 0.23 MB | ~76k |
| `/restore full` | **전문**(무손실) | **전문** | 전문 | 0.62 MB | ~199k |

> 인자 없는 **`/restore` 는 auto** — `handoff.md` 가 있으면 그 이후만(since, 재진입 저렴), 없으면 전체(normal)를 알아서 고르고 어느 쪽인지 출력한다. 전체를 강제하려면 `/restore normal`.

가볍게 읽었다가 부족하면 **`delta`** 로 올라간다 — 발언은 이미 읽었으니 빼고 **도구 블록만** 받는다:

| | 통째로 다시 | **delta** |
|---|---|---|
| `lite` → `normal` | ~76k | **~25k** |
| `lite`/`normal` → `full` | ~199k | **~148k** |

`### ↳ 발언 [N] 이후` 앵커가 붙어 위치가 맞춰진다(발언 번호는 모든 모드 공통). `lite delta` 는 뺄 발언이 없어 성립하지 않으며, 그렇게 실행하면 **무시했다고 알려준다**(조용히 바꾸지 않는다).

또한 **압축 직후 재진입**엔 `/restore since` 가 가장 싸다 — `handoff.md` 가 신선하면 그 이후만(마지막 handoff 이후) 정제해 전체를 다시 읽는 재귀 비용을 피한다. 번호 `[N]` 은 전체본과 정합되고 드릴다운 색인도 그대로 붙는다. **인자 없는 `/restore`(auto)가 이걸 자동으로 쓴다.** since 로 뒷부분만 실렸을 때, 그 이전의 옛 얘기를 물으면 지어내지 말고 `restore/restore-full.md` 를 키워드로 `grep -n` 해 그 줄만 꺼내면 된다 — "가볍게 읽고 필요할 때 꺼낸다"가 이렇게 성립한다.
...
`lite` 는 파일 색인에 더해 **활동 지도**를 담는다: ①도구 종류별 집계 ②`⚠ 오류 N건 → 줄 …`(실패·에러를 줄번호로 바로 노출) ③발언마다 `↳ 도구 N건: … → full L…–…`(그 턴이 실행한 도구 요약 + 꺼내볼 줄범위). 파일이 아닌 활동(Bash·검색·에이전트·웹)도 이 지도로 잡히므로 — 실측상 도구의 37~55%가 `file_path` 없이 색인 밖이던 문제를 메운다 — **라이트만으로도 "무엇을 했고 어디서 꺼내는지"가 완전**하다.
...
## 네이티브 auto-memory(`MEMORY.md`)와의 관계

Claude Code에는 Claude가 자동으로 쓰는 프로젝트 메모리 `MEMORY.md`(세션 시작 시 로드, 압축 후 재주입)가 있다. 이 플러그인의 큐레이션 `handoff.md` 와 겉보기엔 겹치지만 **역할이 다르다** — 그래서 **자동 연동하지 않고 나눠 쓴다**:

| | 네이티브 `MEMORY.md` | 이 플러그인 `handoff.md` |
|---|---|---|
| 담는 것 | 프로젝트에 관한 **지속 사실** ("이 프로젝트는 이렇더라") | **한 세션의 재개 상태** ("지금 무엇을 하다 말았나") |
| 수명 | 계속 남음(하나) | 세션 단위 — 끝나면 버려도 됨 |
| 동시성 | 단일 파일(마지막 쓰기 승) | 세션=폴더라 **충돌 없음** |

지속되는 사실은 `MEMORY.md` 에, "이 세션이 하던 작업"은 `handoff.md` 에 남기면 서로 보완한다. 둘을 엮는 자동 동기화는 취약해서 두지 않았다.

## 왜 원본 백업을 git 에 안 넣나

`.handoff/` 는 전역 gitignore(`**/.handoff/`)로 **저장소에서 제외**된다. 대화 원본(`.jsonl`)엔 비밀번호·키·내부 논의가 섞일 수 있어 팀 공유 저장소에 올라가면 안 되기 때문이다. 그래서 원본 백업은 **로컬에만**(자동 최신 5개) 둔다.

git 으로 이력을 남기고 팀과 공유하는 "버전관리 백업"은 이 gitignore 를 풀어야 하므로 **정면 충돌**한다 — 그 순간 위 유출 위험이 다시 열린다. 그래서 **기본으로 제공하지 않는다.** 특정 프로젝트에서 정말 필요하면 그 위험을 감수하고 해당 `.handoff/` 만 무시에서 풀어 직접 opt-in 할 수 있다(권장하지 않음).
